...
// A è il centro del triangolo
// h è l'altezza (i.e. profondita) del triangolo
...
void LCD_DrawFilledTriangle(point A, uint16_t h, uint16_t d, uint16_t color){
	...
	if(A.y < B.y){
		if(A.y < C.y){
			...
			swapPoints(&C, &A); // C è il più in alto 
			if(C.y < B.y) swapPoints(&B, &C); // CBA
			// CAB
		}
	}
	else{
		if(B.y < C.y){ // B è il più in alto
			swapPoints(&A, &B);
