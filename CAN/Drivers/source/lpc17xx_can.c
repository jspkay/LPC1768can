...
void CAN_Init(LPC_CAN_TypeDef *CANx, uint32_t baudrate)
{
	...
	CLKPWR_SetPCLKDiv (CLKPWR_PCLKSEL_CAN1, CLKPWR_PCLKSEL_CCLK_DIV_1);
	CLKPWR_SetPCLKDiv (CLKPWR_PCLKSEL_CAN2, CLKPWR_PCLKSEL_CCLK_DIV_1);
	CLKPWR_SetPCLKDiv (CLKPWR_PCLKSEL_ACF,  CLKPWR_PCLKSEL_CCLK_DIV_1);
...
Status CAN_SendMsg (LPC_CAN_TypeDef *CANx, CAN_MSG_Type *CAN_Msg)
{
	...
	if ( CANx->SR & (1<<2) )
	{
		...
		data = (CAN_Msg->dataA.string[0])|(((CAN_Msg->dataA.string[1]))<<8)|((CAN_Msg->dataA.string[2])<<16)|((CAN_Msg->dataA.string[3])<<24);
		CANx->TDA1 = data;

		/*Write second 4 data bytes*/
		data = (CAN_Msg->dataB.string[0])|(((CAN_Msg->dataB.string[1]))<<8)|((CAN_Msg->dataB.string[2])<<16)|((CAN_Msg->dataB.string[3])<<24);
		CANx->TDB1 = data;
...
Status CAN_ReceiveMsg (LPC_CAN_TypeDef *CANx, CAN_MSG_Type *CAN_Msg)
{
	...
	if((CANx->SR &0x00000001))
	{
		...
		if (CAN_Msg->type == DATA_FRAME)
		{
			...
			data = CANx->RDA;
			*((uint8_t *) &CAN_Msg->dataA.string[0])= data & 0x000000FF;
			*((uint8_t *) &CAN_Msg->dataA.string[1])= (data & 0x0000FF00)>>8;;
			*((uint8_t *) &CAN_Msg->dataA.string[2])= (data & 0x00FF0000)>>16;
			*((uint8_t *) &CAN_Msg->dataA.string[3])= (data & 0xFF000000)>>24;

			/* Read second 4 data bytes */
			data = CANx->RDB;
			*((uint8_t *) &CAN_Msg->dataB.string[0])= data & 0x000000FF;
			*((uint8_t *) &CAN_Msg->dataB.string[1])= (data & 0x0000FF00)>>8;
			*((uint8_t *) &CAN_Msg->dataB.string[2])= (data & 0x00FF0000)>>16;
			*((uint8_t *) &CAN_Msg->dataB.string[3])= (data & 0xFF000000)>>24;
